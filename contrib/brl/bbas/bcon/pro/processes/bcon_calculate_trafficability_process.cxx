...
namespace bcon_calculate_trafficability_process_globals
{
...
//: constructor
bool bcon_calculate_trafficability_process_cons(bprb_func_process& pro)
{
  using namespace bcon_calculate_trafficability_process_globals;
...
//: execute the process
bool bcon_calculate_trafficability_process(bprb_func_process& pro)
{
  using namespace bcon_calculate_trafficability_process_globals;
  // sanity check
  ...
  for(int i = 0, poly_size = x_coords.data_array.size() * 2 - 1; i + 1 < x_coords.data_array.size(); i++){ 
    ...
    vgl_vector_2d<float> road_normal = road_center.normal() * road_width;
  ...
  */  
  ...
  pro.set_output_val<double>(0, output);
